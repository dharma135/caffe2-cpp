...
OperatorDef* NetUtil::AddImageInputOp(const std::string& reader,
                                               const std::string& data,
                                               const std::string& label,
                                               int batch_size) {
  auto op = AddOp("ImageInput",{reader}, {data, label});  
  net_add_arg(*op, "batch_size", batch_size);
  net_add_arg(*op, "is_test", 1);
  net_add_arg(*op, "scale", 28);
  net_add_arg(*op, "crop", 28);
  net_add_arg(*op, "color", 0);
  net_add_arg(*op, "mean", 0.0f);
  net_add_arg(*op, "std", 1.0f);
  //net_add_arg(*op,"num_labels",10);
  //net_add_arg(*op,"use_caffe_datum",1);
  //net_add_arg(*op,"use_gpu_transform",1);
  return op;
...
OperatorDef* NetUtil::AddGivenTensorFillOp(const TensorCPU& tensor,
                                           const std::string& name) {
  auto op = AddOp("GivenTensorFill", {}, {name});
  net_add_arg(*op, "shape", tensor.sizes().vec());
...
OperatorDef* NetUtil::AddStopGradientOp(const std::string& param_in,const std::string& param_out) {
  return AddOp("StopGradient", {param_in}, {param_out});
}
